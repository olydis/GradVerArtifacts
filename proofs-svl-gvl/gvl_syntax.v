...
(* Lemma LiftingPrimitiveLifting : forall f, IdealDomainNonSingularCodomain f -> Deterministic f -> Monotone f -> SatisfiableCodomain f ->
  LiftFunction1 f (PrimitiveLift f).
Proof.
  intro. intro insc. intro det. intro mon. intro satcd.
  reduce.
  split; intro hyp.
  - inv hyp; reduce.
    * eexists. split. eex.
      eexists. split. eauto.
      exists (fun phi' => ∃ phi0 : Set_Formula,
         f phi0 phi' ∧ MakePFormSingleton phi1 phi0). split. reduce. split; auto.
      reduce. split.
      + eex. reduce. unf. inv H2. eapply det in H0; eauto.
      + intro. intro hyp. unf.
        reduce. eex.
        intro. intro mpfi. apply H2.
        inv mpfi. eex.
    * eexists. split. eex.
      eexists. split. eauto.
      exists (fun phi' => ∃ phi0 : Set_Formula, f phi0 phi' ∧ MakePFormIdeal phi1 phi0).
      split. reduce. split; auto.
      reduce. split.
      + exists (MakePFormIdeal phi2). split. eapply GFormEqualSameGamma; eauto.
        reduce. unf. split. Focus 2. eapply satcd; eauto.
        inv H3. eapply mon; eauto.
      + intro. intro hyp. unf.
        reduce.
        exists (MakePFormIdeal phi2).
        exists x.
        split. eapply GFormEqualSameGamma; eauto.
        split; auto.
        intro. intro mpfi.
        inv H2.
        --apply except.
          inv H1.
          eapply (PFormSubsetTrans (fun phi' => ∃ phi0 : Set_Formula, _)) in H2; eauto.
          reduce in H2.
          assert (C1 := H2 phi2).
          assert (H' := H).
          apply insc in H'; auto. unf.
          assert (C2 := H2 x1).
          lapply C1. Focus 2. eex.
          intro c1.
          lapply C2. Focus 2. eexists x0. split. auto. split; auto.
          intro c2.
          subst. congruence.
        --apply H1.
          inv mpfi.
          ctor; auto.
          inv H1. eapply ImpliesTrans; eauto.
          eapply (PFormSubsetTrans (fun phi' => ∃ phi0 : Set_Formula, _)) in H5; eauto.
          specialize (H5 phi2). lapply H5. Focus 2. eex.
          intro c. apply c.
  - inv hyp. unf.
    apply H0 in H2. clear H0.
    reduce in H2.
    assert (exists p, x1 p) as has. eapply AlphaNotEmpty; eauto.
    inv has.
    apply H2 in H. unf.
    inv H3. unf.
    inv H1.
    * apply H3 in H4. inv H4.
      specialize (H5 (GFormPrecise x2)).
      lapply H5. Focus 2. eex. intro. intro c. reduce. apply H2 in c. unf. apply H3 in H7. inv H7. eapply det; eauto.
      intro c.
      eapply MorePreciseThanPrecise in c. subst.
      ctor. assumption.
    * apply H3 in H4. inv H4.
      assert (exists x6, f phi x6). admit. unf.
      assert (Satisfiable x5) as sat'. eapply mon in H1; eauto. inv H5. eex.
      apply (PrimitiveLiftImprecise _ _ _ _ sat'). split; auto.
      split.
      + inv H.
        --apply except.
          inv H9.
          assert (PFormSubset (fun phi' => ∃ phi1 : Set_Formula,
              (SubstFormVarExpr x e phi1 phi' ∧ Satisfiable phi') ∧ x1 phi1) (MakePFormSingleton phi0)) as pfs.
            intro. intro c. apply H2 in c. apply H7 in c. apply H in c. assumption.
          reduce in pfs.
          assert (C1 := pfs x6).
          assert (C2 := pfs (FormAnd x6 FormTrue)).
          lapply C1. Focus 2. eex. apply H3. auto.
          intro c1.
          lapply C2. Focus 2. exists (FormAnd phi FormTrue). split.
            split; auto. inv sat'. eex.
            apply H3. reduce. split.
              intro. intro c. inv c. assumption.
              inv sat. eex.
          intro c2.
          subst. apply RecFormAndL in c2. auto.
        --apply MorePreciseThanImprecise.
          inv H9.
          assert (x2 x6) as c. apply H2. eex. apply H3. auto.
          apply H7 in c. apply H in c. apply c.
      + apply H6.
        eexists. split. eauto.
        intro. intro c.
        apply H2 in c. unf. apply H3 in H11. inv H11.
        eapply MonotoneSubstFormVarExpr in H10; eauto.
Qed.
 *)
...
(* Lemma LiftingSubstGFormVarExpr : forall x e, LiftFunction1 (fun phi1 phi2 => SubstFormVarExpr x e phi1 phi2 /\ Satisfiable phi2) (SubstGFormVarExpr x e).
Proof.
  reduce.
  split; intro hyp.
  - inv hyp; reduce.
    * unf.
      ...
      eexists. split. eauto.
      exists (fun phi' => ∃ phi0 : Set_Formula,
         (SubstFormVarExpr x e phi0 phi' ∧ Satisfiable phi')
         ∧ MakePFormSingleton phi1 phi0). split. reduce. split; auto.
      reduce. split.
      + eex. reduce. unf. inv H3. eapply SubstFormVarExprDet in H0; eauto.
      + intro. intro hyp. unf.
        reduce. eex.
        intro. intro mpfi. apply H3.
        inv mpfi. eex.
    * eexists. split. eex.
      eexists. split. eauto.
      exists (fun phi' => ∃ phi0 : Set_Formula, (SubstFormVarExpr x e phi0 phi' ∧ Satisfiable phi') ∧ MakePFormIdeal phi1 phi0).
      split. reduce. split; auto.
      reduce. split.
      + exists (MakePFormIdeal phi2). split. eapply GFormEqualSameGamma; eauto.
        reduce. unf. split; auto.
        reduce. inv H3. eapply MonotoneSubstFormVarExpr in H6; eauto.
      + intro. intro hyp. unf.
        reduce.
        exists (MakePFormIdeal phi2).
        exists x0.
        ...
        --apply except.
          ...
          reduce in H2.
          assert (C1 := H2 phi2).
          assert (C2 := H2 (FormAnd phi2 FormTrue)).
          lapply C1. Focus 2. eex.
          intro c1.
          lapply C2. Focus 2. exists (FormAnd phi1 FormTrue). split.
            split; auto. inv H4. eex.
            reduce. split.
              intro. intro c. inv c. assumption.
              inv sat1. eex.
          intro c2.
        ...
        --apply H.
          inv mpfi.
          ctor; auto.
          inv H. eapply ImpliesTrans; eauto.
          eapply (PFormSubsetTrans (fun phi' => ∃ phi0 : Set_Formula, _)) in H6; eauto.
          specialize (H6 phi2). lapply H6. Focus 2. eex.
          intro c. apply c.
  - inv hyp. unf.
    apply H0 in H2. clear H0.
    reduce in H2.
    assert (exists p, x2 p) as has. eapply AlphaNotEmpty; eauto.
    inv has.
    apply H2 in H. unf.
    inv H3. unf.
    inv H1.
    * apply H3 in H4. inv H4.
      specialize (H6 (GFormPrecise x3)).
      lapply H6. Focus 2. eex. intro. intro c. reduce. apply H2 in c. unf. apply H3 in H8. inv H8. eapply SubstFormVarExprDet; eauto.
      intro c.
      eapply MorePreciseThanPrecise in c. subst.
      ctor. split; auto.
    * apply H3 in H4. inv H4.
      assert (tot := SubstFormVarExprTotal x e phi). unf.
      assert (Satisfiable x6) as sat'. eapply MonotoneSubstFormVarExpr in H1; eauto. inv H5. eex.
      apply (PrimitiveLiftImprecise _ _ _ _ sat'). split; auto.
      split.
      + inv H.
        --apply except.
...
(*XMonotoneGFunction1 is hybrid between Monotone and IMonotone:
  - subst is not Monotone (see example somewhere below) but IMonotone
  - Call is not IMonotone (max { ... } not guaranteed to exist if weaker formula as input) but Monotone
*)

(* Lemma MonotoneGFunction1XMonotone : forall gf, MonotoneGFunction1 gf -> XMonotoneGFunction1 gf.
Proof.
  intro. intro mon.
  intro. intro. intro mpt. intro. intro gfx.
  eapply mon in gfx; eauto.
Qed. *)

(* 
Lemma IMonotonePrimitiveLift : forall f, MonotoneDef f -> Monotone f -> IMonotoneGFunction1 (PrimitiveLift f).
Proof.
  intro. intro mond. intro mon. intro. intro. intro mit. intro. intro pl.
  apply MoreImpliesThanAltEq in mit.
  destruct gp1, gp2; inv pl.
  - lapply (mit s s0); auto. intro c.
    lapply c; auto. intro d.
    assert (exists sx, f s0 sx) as ee. eapply mond in d; eauto. unf.
    eapply mon in d; eauto.
    exists (GFormPrecise x).
    split; auto.
    apply MoreImpliesThanAltEq. intro. intro. intro s1. intro s2.
    inv s1. inv s2.
    assumption.
  - lapply (mit s phi); auto. intro c.
    lapply c; auto. intro d.
    assert (exists sx, f phi sx) as ee. eapply mond in d; eauto. unf.
    eapply mon in d; eauto.
    eexists (GFormImprecise x _).
    split; auto. ctor; eauto.
    apply MoreImpliesThanAltEq. intro. intro. intro s1. intro s2.
    inv s1. inv s2.
    assumption.
  .
...
(* Lemma IMonotoneLiftFunction1 : forall f gf,
  LiftFunction1 f gf -> WeakPartialGC3 (MakePLiftFunction1 f) -> IMonotoneGFunction1 gf.
Proof.
  intro. intro. intro lf. intro pg. intro. intro. intro mpt. intro. intro gff.
  apply lf in gff.
  reduce in gff. unf.
  apply H0 in H2. clear x H0.
  apply MoreImpliesThanAlt2Eq in mpt.
  reduce in mpt.
  assert (tot := GammaTotal gp2). unf.
  assert (tot := StaticTotal gp1). unf.
  specialize (mpt x0 x H1 H x2 (GammaContainsStatic gp1 x0 x2 H0 H1)).
  unf.

  assert (tot := MakePLiftFunction1Total f x). unf.
  assert (∃ gphi' : Set_GFormula, Alpha x4 gphi') as ee. admit.
  ...
  - apply MoreImpliesThanAltEq.
    ...
    exists x6. split. eapply GammaContainsStatic; eauto.
    eex.
...
(* Lemma MonotoneSubstGFormVarExpr : forall x e, MonotoneGFunction1 (SubstGFormVarExpr x e).
Proof.
  intro. intro.
  eapply MonotoneLiftFunction1. eapply LiftingSubstGFormVarExpr.
  (* WEAK PARTIAL GC3 *)
  intro. intro. intro ga. intro. intro mpl. intro ne.
  inv ga.
  - assert (tot := SubstFormVarExprTotal x e phi). unf.
    exists (GFormPrecise x0).
    split.
    * eex.
      intro. intro pp.
      apply mpl in pp. unf.
      apply H in H3. inv H3.
      eapply SubstFormVarExprDet in H0; eauto.
    * intro. intro ee. unf.
      eex.
      intro. intro pp. inv pp.
      apply H3.
      inv ne. apply mpl in H1. apply mpl.
      unf. apply H in H5. inv H5.
      eapply SubstFormVarExprDet in H0; eauto. subst.
      eex.
      apply H. ctor.
  - assert (tot := SubstFormVarExprTotal x e phi). unf.
    assert (Satisfiable x0) as sat2.
      inv ne. apply mpl in H1. unf.
      apply H in H3. inv H3.
      eapply MonotoneSubstFormVarExpr in H1; eauto.
      inv H4. apply H1 in H3. eex.
    exists (GFormImprecise x0 sat2).
    clear ne.
    split.
    * eexists. split. eauto.
      intro. intro pp.
    ...
    * intro. intro ee. unf.
      eex.
      intro. intro pp. inv pp.
      assert (pphi' x0) as p1.
        apply mpl. eex. apply H. eauto.
      assert (pphi' (FormAnd FormTrue x0)) as p2.
        inv sat2.
        apply mpl. eex. apply H. reduce. split.
          intro. intro ef. inv ef. assumption.
          inv sat. eexists. ctor; eauto.
      apply H3 in p1.
      apply H3 in p2.
      clear H3.
      inv H2. apply H3 in p1. apply H3 in p2. inv p1. inv p2. apply RecFormAndR in H2. inv H2.
      apply H3. apply H3 in p1. apply H3 in p2.
      inv p1. inv p2.
      eex.
      eapply ImpliesTrans; eauto.
Qed.
...
(* Definition GDeterministic (f : Set_GFormula -> Set_GFormula -> Prop) := forall phi phi1 phi2, f phi phi1 -> f phi phi2 -> GFormEqual phi1 phi2.
Lemma PrimitiveLiftDet : forall f, Deterministic f -> GDeterministic (PrimitiveLift f).
Proof.
  intro. intro det. intro. intro. intro. intro p1. intro p2.
  inv p1; inv p2.
  - assert (s := det _ _ _ H H2). subst.
    auto.
  - assert (s := det _ _ _ H H2). subst.
    auto.
  - assert (s := det _ _ _ H H2). subst.
  ...
  - assert (s := det _ _ _ H H2). subst.
    auto.
    contradict H4. assumption.
  - assert (s := det _ _ _ H H2). subst.
    inv H0. inv H4.
    inv H1. inv H3. inv H0. inv H5. unf.
    inv H4. inv H3. inv H8. inv H5.
    eapply GammaDet in H1; eauto.
    eapply GammaDet in H0; eauto.
    split.
    * exists x0. exists x2.
      repeat split; auto.
      intro. intro xx.
      apply H0. apply H11. apply H4.
      apply H9 in xx. apply H12 in xx.
      assumption.
    * exists x2. exists x0.
      repeat split; auto.
      intro. intro xx.
      apply H1. apply H7. apply H14.
      apply H13 in xx. apply H8 in xx.
      assumption.
Qed. *)
...
(* Lemma GWPDet : forall s, GDeterministic (GWP s).
Proof.
  induction s; intro; intro; intro; intro; intro h1; intro h2; intro eq; inv h1; inv h2.
  - auto.
  - eapply (SubstGFormVarExprDet s s0 phi1' phi2' gphi' gphi'0) in eq; eauto.
    inv eq.
    * eapply GFormAndDet in H; eauto.
    * apply or_intror. unf.
      split.
      + intro. contradict H0.
        inv H5; try (eex; fail).
        inv H. inv H0. inv H. inv H5. inv H. eex.
      + intro. contradict H6.
        inv H8; try (eex; fail).
        inv H. inv H6. inv H. inv H7. inv H. eex.
  - eapply LookupMethodDet in H5; eauto. inv H5.
    clear H2 H3 H4 H8 H9 H12. (* dups (remove both) *)
    assert (eq1 := GFormEqualRefl phi0).
    assert (eq2 := GFormEqualRefl phi3).
    assert (det1 := SubstGFormVarExprDet _ _ _ _ _ _ H6 H14 eq1).
    assert (det2 := SubstGFormVarExprDet _ _ _ _ _ _ H7 H15 eq2).
    clear eq1 eq2 H6 H7 H14 H15.
    assert (GFormEqual phi2'0 phi2'1 ∨ ¬ GSatisfiable phi2'0 ∧ ¬ GSatisfiable phi2'1) as det3.
      inv det2.
        eapply SubstGFormVarExprDet; eauto.
      apply or_intror. apply and_not_or in H. apply not_or_and. intro c. contradict H.
      inv c.
        apply or_introl. eapply SubstGFormVarExprGSatisfiable; eauto.
        apply or_intror. eapply SubstGFormVarExprGSatisfiable; eauto.
    clear H10 H18.
    assert (IsPrecise phi1'0 <-> IsPrecise phi1'1) as ip1.
      apply XGFormEqualSamePrec. auto.
    assert (IsPrecise phi2'' <-> IsPrecise phi2''0) as ip2.
      apply XGFormEqualSamePrec. auto.
    assert (IsPrecise phi2'0 <-> IsPrecise phi2'1) as ip3.
      apply XGFormEqualSamePrec. auto.
    inv det1.
    {
      admit.
    ...
    {
      inv H.
      apply NotGSatisfiable in H0.
      apply NotGSatisfiable in H1.
      unf.
      
      inv H11. inv H19. unf.
      clac (IsPrecise phi2'); clac (IsPrecise phi2'1).
      - apply or_intror.
        clear H9 H10.
        assert (IsPrecise phi1').
          inv H11. inv eq. apply MorePreciseThanPrecise in H9. subst. eex.
        assert (IsPrecise phi2'0).
          apply H0. assumption.
        lapply H8. Focus 2. repeat split; auto. eex. intro mi1. clear H8.
        lapply H7. Focus 2. repeat split; auto. eex. intro mi2. clear H7.
        unf.

        split.
        * intro c. contradict H2. inv c. inv H2. inv H7. inv H15. inv H2. eex.
          inv H8. unf. inv H2. inv H20. apply H21. eauto.
        * intro c. contradict H1. inv c. inv H1. inv H7. inv H15. inv H1. eex.
          inv H14. unf. inv H1. inv H20. apply H21. eauto.
      - apply except.
        clear H8 H7.
        lapply H9. Focus 2. repeat split; auto. intro c. unf. congruence. intro mi1. clear H9.
        unf.
  ...
  - assert (IsPrecise phi1' <-> IsPrecise phi2') as ip.
      apply XGFormEqualSamePrec. auto.
    unf.
    destruct phi1'; destruct phi2'.
      Focus 2. lapply H. intro. inv H3. inv H4. eex.
      Focus 2. lapply H2. intro. inv H3. inv H4. eex.
    * inv H0. inv H1.
      inv eq. apply MorePreciseThanPrecise in H0. inv H0.
      ctor. auto.
    * eapply GFormEqualSameStatic in eq; eauto.
      inv H0; inv H1.
      + apply or_intror.
        split; intro c; inv c; inv H0; inv H1; congruence.
      + contradict H6.
        clear H5. inv sat2. inv H0. eex. ctor; eauto. apply eq. assumption.
      + contradict H5.
        clear H6. inv sat2. inv H0. eex. ctor; eauto. apply eq. assumption.
      + ctor.
        apply GFormEqualSymm in H6.
        eapply GFormEqualTrans; eauto.
        eapply GFormEqualTrans in H5; eauto.
        split; apply MorePreciseThanImprecise; apply ImpliesAnd; auto; apply eq.
  - eapply IHs2 in eq. Focus 2. eauto. Focus 2. eauto.
    inv eq.
    * eapply IHs1; eauto.
    * unf.
      assert (XMorePreciseThan phi3 phi4) as x1.
        apply or_intror. apply or_introl.
        apply NotGSatisfiable in H0.
        apply NotGSatisfiable in H3. unf.
        eex; intro; intro ef.
          contradict H5. eex.
          contradict H3. eex.
      assert (XMorePreciseThan phi4 phi3) as x2.
        apply or_intror. apply or_introl.
        apply NotGSatisfiable in H0.
        apply NotGSatisfiable in H3. unf.
        eex; intro; intro ef.
          contradict H3. eex.
          contradict H5. eex.
      assert (MorePreciseThan phi1 phi2) as m1.
        eapply GWpXMonotone in H4; eauto. unf.
        eapply (IHs1 phi4 phi4) in H6; eauto.
      Print XMonotoneGFunction1.
      Print XMorePreciseThan.

      apply or_introl.
      
contradict H12. apply .
          apply H8 in H17.

        inv H8. unf. inv H7. inv H19.
        inv H14. clear H7. apply H16 in H8.
        apply H14 in H8.
        lapply (H15 phi2).
          intro c. apply H2. exists VarEnvFresh. apply H20. apply c. ctor.
        split. intro. intro c. inv c.
        
      clac x1; clac x2; apply H7 in H9; apply H8 in H10; clear H7 H8; unf.
      Focus 2. 

    }
    inv H11. inv H19.
    clac x; clac x0; apply H in H1; apply H0 in H2; clear H H0; unf.
    * inv det1.
      {
        admit.
      }
...
(* x; s  ->  s  ->  x *)
Inductive ConsStmt : Set_Stmt -> Set_Stmt -> Set_Stmt -> Prop :=
| PopStmtSingle : forall x,
  x <> StmtSkip ->
  (forall a b, x <> StmtSeq a b) ->
  ConsStmt x StmtSkip x
| PopStmtSeq : forall s1 s1' s2 s,
  ConsStmt s1 s1' s ->
  ConsStmt (StmtSeq s1 s2) (StmtSeq s1' s2) s
| PopStmtSeqSkip : forall s2,
  ConsStmt (StmtSeq StmtSkip s2) s2 StmtSkip
.

Hint Constructors ConsStmt.
...
Lemma ConsStmtCodomain : forall b, (exists s a, ConsStmt s a b) <-> (forall x y, b <> StmtSeq x y).
Proof.
  split; intro hyp.
  - unf.
    induction H0.
  ...
  - generalize b hyp.
    clear.
    induction b; intro hyp.
    * exists (StmtSeq StmtSkip StmtSkip).
      eex.
    * eex.
      ctor; congruence.
    * eex.
...
Lemma ConsStmtFirst : ∀ s12 s1 s2, ConsStmt s12 s2 s1 -> FirstStmt s12 s1.
Proof.
  induction s12; intros; inv H;
  try congruence;
  try (ctor; intro c; unf; congruence; fail).
  - ctor. eapply IHs12_1. eauto.
  - ctor. ctor; intro c; unf; congruence.
Qed.
...
Proof.
  ...
  - assert (ValidMethod p (Method _T1 m _T2 i (Contract phi _phi) r)) as vam.
      reduce in va. inv H1. reduce in H. destruct p. unf. apply H4 in H1. assumption.
    inv vam. unf.
    unf. rename H4 into WPexists.
    assert (tot := gdiffTotal (GFormPrecise phi) (GFormPrecise x0)). unf. rename H into tot.

    ctor.
    Focus 6. eauto.
    Focus 4. eapply LookupGMethodCoincidesWithLookupMethod in H1; eauto.
    Focus 5. exists phi. eauto.
    Focus 5. ctor. congruence. intros. congruence.
    Focus 4. assumption.
    ...
    apply H8 in H5.
    ctor. split; eauto.
    eapply SubstFormSemantics in H5; eauto.
  ...
  - eapply GProgramMorePreciseThanLookup in H6; eauto. unf.
    ...
    * ctor.
      ...
      Focus 1.
        ...
        ctor.
          admit. (* SP supposed to hold by def... no lemma yet *)
...
(*
Lemma gdiffMonHelper : forall a1 b1 a2 b2 a3, MoreImpliesThan b1 a1 -> MoreImpliesThan a2 b2 -> gdiff a1 a2 a3 -> exists b3, MoreImpliesThan a3 b3 /\ gdiff b1 b2 b3.
Proof.
  intro. intro. intro. intro. intro. intro mpt. intro xmpt. intro gd.
  inv gd. unf.
  apply PrimitiveLiftStatic in H1. unf.
  assert (tot := StaticTotal b1). unf.
  assert (tot := StaticTotal b2). unf.
  apply MoreImpliesThanAltEq in mpt.
  apply MoreImpliesThanAltEq in xmpt.
  specialize (mpt _ _ H0 H3).
  specialize (xmpt _ _ H1 H5).
  assert (MonotoneDiff diff) as mon. apply diffValid.
  assert (DefDiff diff) as tot. apply diffValid. specialize (tot x3 x0). unf.
  assert (DefDiff diff) as tot. apply diffValid. specialize (tot x3 x4). unf.

  assert (Implies x1 x6) as im.
    assert (m1 := mon x0). apply proj2 in m1.
    assert (m2 := mon x3). apply proj1 in m2.
    eapply m1 in mpt. Focus 2. eauto. Focus 2. eauto.
    eapply m2 in xmpt. Focus 2. eauto. Focus 2. eauto.
    eapply (mon x3) in H6; eauto. apply H6 in H7; eauto.


  destruct b2.
  - inv H2. eex.
    apply MoreImpliesThanAltEq. intro. intro. intro s1. intro s2. inv s2.
    assert (tot := StaticTotal a2). unf.
    unf.
    intro. intro ef.
    assert (Implies x x0). apply MoreImpliesThanAltEq in mpt. eapply mpt; eauto.
    eapply mon.
    eapply diffValid.
    reduce.

 inv H1.
  - apply XMorePreciseMoreImplies in xmpt. apply MoreImpliesThanAltEq in xmpt. assert (tot := StaticTotal b2). unf. lapply (xmpt phi1 x0).
    * intro c. apply c in H1.
...
(* the more this implies, the more information content, so the more terms...
   note that for a measure actually counting terms + a monotonicity theorem on, you need a NF
   but that normal form would exactly be larger the more information a term carries *)
Definition CostsMoreThan := MoreImpliesThan.

(* Theorem PayAsYouGo  *)

(* need ideal GSP for that. *)
