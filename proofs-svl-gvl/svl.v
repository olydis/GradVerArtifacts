...
(* | StmtDeclare : Set_Type -> Set_Var -> Set_Stmt *)(* just special case of VarAssign *)
...
| StepCall : ∀ (r : Set_Stmt) (phi : Set_Formula) (rho rho' : Set_VarEnv) (S : Set_Stack) (x y : Set_Var) (i : Set_Ident) (v : Set_Val) (m : Set_MethodName) (_T1 _T2 : Set_Type) (_phi : Set_Formula) S1 S2,
  ...
  Step
    ...
    (StackCons (StackFrame rho' r) (StackCons (StackFrame rho S1) S))
| StepCallFinish : ∀ (_r : Set_Stmt) (phi : Set_Formula) (rho rho' : Set_VarEnv) (S : Set_Stack) (x y : Set_Var) (i : Set_Ident) (v  : Set_Val) (m : Set_MethodName) (T _T : Set_Type) (_phi : Set_Formula) S1 S2,
  ...
  Step
    ...
    (StackCons (StackFrame (VarEnvMapping y v rho) S2) S)
...
Lemma ConsStmtWP : forall s12 s1 s2, ConsStmt s12 s2 s1 -> forall a c, WP s12 c a <-> exists b, WP s2 c b /\ WP s1 b a.
Proof.
...
Proof.
  ...
  - destruct frame as [rho s].
    ...
    * destruct pi.
      ...
      + clear not_final.
        ...
        **assert (exists v, EvalVar x VarResult v) as exx.
          ...
          inv exx.
          eexists; eapply StepCallFinish; eauto. ctor. congruence. intros. congruence.
        **apply IHx0_1 in H4.
          unf.
          inversion H; subst.
          ++inv H10.
          ++eexists.
            eapply StepCallFinish;
            eauto.
    * clear not_final.
      inv valid.
      unf.
    ...
    * clear not_final.
      ...
      --ctor. congruence. intros. congruence.
    * clear not_final.
      inv valid.
      unf.
      clear H3.
...
Proof.
  ...
  - symmetry in H4. apply RecStack1 in H4. inv H4.
  - apply RecStack1 in H5. inv H5.
  - apply RecStack1 in H5. inv H5.
  - apply RecStack1 in H5. inv H5.
  - apply RecStack1 in H5. inv H5.
  - eapply StepSeq; eauto.
Qed.
...
Proof.
  ...
  assert (exists p, WP s _phi p /\ forall rho, EvalVar rho (VarOldIdent i) v -> EvalVar rho (VarIdent i) v -> EvalFormula rho phi -> EvalFormula rho p) as ex.
    ...
    destruct p. unf. apply H0 in H4.
    inv H4. unf.
    eexists.
    split. eauto.
    intro. intro ev1. intro ev2. intro ef.
    apply H13 in ef.
    eapply SubstFormSemantics in H9; eauto.
    Focus 2. ctor. eauto.
    apply H9 in ef.
    assert (fvl := FreeVarFormUpperBound x2). unf.
    eapply ObservablyIdenticalVarEnvEvalFormula; eauto.
    intro. intro inx. intro.
    split; intro evar.
      clac (x5 = VarOldIdent i).
...
Lemma ConsStmtDet : forall S1 S2 s1 s2, ConsStmt S1 S2 s1 -> ConsStmt S1 S2 s2 -> s1 = s2.
Proof.
  induction S1; intro; intro; intro; intro c1; intro c2;
  inv c1; inv c2; auto.
  - specialize (H0 StmtSkip StmtSkip). congruence.
  - eapply IHS1_1; eauto.
  - inv H3. auto.
  - specialize (H0 StmtSkip StmtSkip). congruence.
  - inv H3. auto.
...
Proposition PreservationExit : ∀ S1 S2 y m x s rho rho' rho'' _phi1 phi2 i _T1 _T2 v,
  Step (StackCons (StackFrame rho'' StmtSkip) (StackCons (StackFrame rho S1) StackEmpty))
       (StackCons (StackFrame rho' S2) StackEmpty) ->
  ConsStmt S1 S2 (StmtCall y m x) ->
...
Proof.
  ...
  intro step. intro fst. intro ev1. intro ev2. intro lookup. intro.
  intro pre.
  unf.
  inv step.
  inv H0.
  eapply LookupMethodDet in lookup; eauto. inv lookup.
  eapply ConsStmtDet in fst; eauto. inv fst. assert (fst := H10). apply ConsStmtFirst in fst.
  eapply LookupMethodDet in H6; eauto. inv H6.
  inv H17. apply H. (*MaxImplies*)
  ctor.
  - inv H1.
  ...
  - (* first subst *)
    assert (EvalExpr (VarEnvMapping y v0 rho) (ExprVar y) v0) as tmp.
      ctor. ctor.
    eapply SubstFormSemantics in H16; eauto.
    symmetry in H16.
    rewrite H16.
    clear H16 tmp.
    (* second subst *)
    assert (exists vv, EvalExpr (VarEnvMapping VarResult v0 (VarEnvMapping y v0 rho)) (ExprVar x) vv /\ EvalVar rho'' (VarOldIdent i0) vv) as tmp.
      inv H1. inv H15. exists v1. inv H16.
      split.
        ctor. ctor; try congruence. ctor; congruence.
        eapply EvalVarDet in ev1; eauto.
        subst. assumption.
    inv tmp. inv H2.
    eapply SubstFormSemantics in H13; eauto.
    symmetry in H13.
    rewrite H13. clear H13.
    (* obs. eq. *)
    eapply (ObservablyIdenticalVarEnvEvalFormula [(VarOldIdent i0); VarResult]); eauto.
    * inv H3. inv H14; try congruence. inv H18; try congruence.
      intro gx. intro ingx. intro vv.
      split; intro ev.
      + inv ev; auto.
        inv ingx; try congruence. inv H2. Focus 2. inv H3.
        inv H20; try intuition.
      + inv ingx.
        --eapply EvalVarDet in H6; eauto. subst. ctor.
        --inv H2; try (intuition; fail).
          ctor; try congruence.
          eapply EvalVarDet in H8; eauto. subst. ctor.
    * intro. intro fvf.
      assert (va := Validity).
      unfold ValidProgram in va.
      rename H11 into lookup. inv lookup. unfold LookupMethodAmbig in *.
      destruct p. unf. apply H15 in H13.
      reduce in H13.
      apply H13 in fvf. inv fvf; intuition.
Qed.
...
Proof.
  intro; intro; intro; intro; intro; intro step; intro; intro valid; inv step;
  ...
  - split. auto.
    ...
    split. ctor.
    split. ctor. intro c. unf. inv H2.
    eexists. eexists. eexists. eexists. eexists.
    split. eauto.
    split. Focus 2. eex. ctor.

    assert (val := Validity).
    unfold ValidProgram, LookupMethod, LookupMethodAmbig in *. destruct p. unf. apply H2 in H4. inv H4. unf. ctor. eauto.
    apply H14 in H7. eapply SubstFormSemantics in H10. apply H10 in H7. Focus 2. ctor. ctor; try congruence. ctor.
    assert (fvf := FreeVarFormUpperBound x1). unf.
    eapply ObservablyIdenticalVarEnvEvalFormula; eauto.
  ...
  - split. auto.
    eexists. eexists. eexists. eexists. eexists.
    split. ctor.
    split. ctor. eapply ConsStmtFirst. eauto.
    eexists. eexists. eexists. eexists. eexists.
    split. eauto.
    split. Focus 2. eex. ctor.

    assert (val := Validity).
    unfold ValidProgram, LookupMethod, LookupMethodAmbig in *. destruct p. unf. apply H1 in H4. inv H4. unf. ctor. eauto.
    apply H13 in H7. eapply SubstFormSemantics in H9. apply H9 in H7. Focus 2. ctor. ctor; try congruence. ctor.
    assert (fvf := FreeVarFormUpperBound x1). unf.
    eapply ObservablyIdenticalVarEnvEvalFormula; eauto.
    intros. split; intro ev; inv ev; ctor; auto.
    * ctor; auto.
...
Lemma StepExitDoesntAlterOldVariable : forall s s' rho rho' sf i v phi1 phi2,
  ...
  EvalVar rho' (VarOldIdent i) v.
Proof.
  induction s; intro; intro; intro; intro; intro; intro; intro; intro; intro wp; intro step; intro ev; inv step; inv H8.
  - inv wp.
    ctor; eauto.
  - inv wp.
    eapply IHs1 in H8; eauto.
    ctor; eauto.
Qed.
...
Lemma PreservationExitEx : ∀ sf'' sf sf' pi,
  ...
  (ValidStateEx p (StackCons sf pi)).
Proof.
  intro. intro. intro. intro. intro step. intro. intro valid.
  destruct sf'. destruct sf''.
  destruct sf.
  apply StepLocalityExit in step.
  assert (s0 = StmtSkip). inv step. auto. subst.
  inv valid.
  inv H4.
  assert (ConsStmt s2 s4 (StmtCall _y m x)). inv step. assert (co := H14). apply ConsStmtFirst in H14. eapply FirstStmtDet in H7; eauto. inv H7. assumption.
  inv H11.
  - eapply ConsStmtWP in H3; eauto. unf.
    ctor; eauto.
  ...
  - eapply ConsStmtWP in H3; eauto. unf.
    ...
    * inv step.
      ctor; auto.
      eapply ConsStmtDet in H; eauto. inv H.
      inv H2.
      intro c. contradict H11. eex.
Qed.
